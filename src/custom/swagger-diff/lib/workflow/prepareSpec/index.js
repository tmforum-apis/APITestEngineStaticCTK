...
function prepareSpec (spec) {
  ...
  return (0, _dereference2.default)(specRef).then(function (dereferencedSpec) {
    debug('dereferenced');
    var specs = JSON.parse(JSON.stringify(dereferencedSpec));

    //var specs = dereferencedSpec; //AVTS-283 - shallow copy creating a memory corruption down the line in the inlineReuiredProperties
