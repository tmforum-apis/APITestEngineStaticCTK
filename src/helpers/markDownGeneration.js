...
function displayRuleList(ruleList, type, emoj){
	let title = '##  ' + type + '\n' + '\n' ;
	let header = '| ruleId | message  | path  |  property |' + '\n'
	header +=    '|:-------|:---------|:------|:----------|' + '\n'
	let rows = '';
...
function displayTitle(compliance){
	...
	switch(compliance){
		case FAIL:
			result ='# API  Compliance Results :x:' +'\n' + '\n';
			break;
		case WARNING:
			result ='# API  Compliance Results :warning:' +'\n' + '\n';
			break;
		case PASS:
			result ='# API  Compliance Results :white_check_mark:' +'\n' + '\n';
			break;
		default:
...
*/
exports.generateMarkDown= function (jsonObj){
	...
	if(jsonObj.hasOwnProperty('results') && jsonObj.results.hasOwnProperty('rules')){
		...
		if(rules.hasOwnProperty('errors') && rules.errors.length > 0){
			errors = displayRuleList(rules.errors, 'Rules which caused COMPATIBILITY issuess :x:', ':x:') + '\n';
		}
		if(rules.hasOwnProperty('warnings') && rules.warnings.length > 0){
			warnings = displayRuleList(rules.warnings, 'Rules which caused WARNINGs :warning:', ':warning:') + '\n';
		}
